...
export function ThemeProvider({ children }: { children: React.ReactNode }) {
  ...
  const loadGlobalSettings = async () => {
    ...
    try {
      ...
      try {
        console.log(
          "🎨 [THEME] Attempting to load global settings with ID:",
          GLOBAL_SETTINGS_ID
        );
        ...
        console.log(
          "⚠️ [THEME] Error loading global settings, using defaults:",
          error
        );
      }
      ...
      try {
        ...
        if (superadminData) {
          ...
          console.log(
            "✅ [THEME] Superadmin settings loaded:",
            superadminSettings
          );
        } else {
          console.log(
            "⚠️ [THEME] No superadmin settings found, using defaults"
          );
        }
      } catch (error) {
      ...
      const finalSettings = {
        ...
        project_name: superadminSettings?.project_name || "Civet",
        api_endpoint:
          superadminSettings?.api_endpoint ||
          "https://cibet.app.n8n.cloud/webhook/invitu-public-upload",
      };
  ...
  const updateSettings = async (updates: Partial<UserSettings>) => {
    ...
    try {
      ...
      if (isSuperAdmin) {
        ...
        console.log(
          "💾 [THEME] User updating personal theme:",
          personalUpdates
        );
  ...
  const updateLogo = async (file: File): Promise<void> => {
    return new Promise((resolve, reject) => {
      ...
      if (!allowedTypes.includes(file.type)) {
        reject(
          new Error("Formato de archivo no soportado. Use JPG, PNG o SVG.")
        );
        return;
      }
  ...
  const primaryColor =
    ...
    colorSchemes[settings.color_scheme as keyof typeof colorSchemes] ||
    colorSchemes.black;
